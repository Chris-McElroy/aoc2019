...
for dir in line2:
    dist = string.atoi(dir[1:])
    prevCoords = line2coords[-1]
    if dir[0] == 'R':
        line2coords += [(prevCoords[0]+dist, prevCoords[1])]
        
    elif dir[0] == 'L':
        line2coords += [(prevCoords[0]-dist, prevCoords[1])]
        
    elif dir[0] == 'U':
        line2coords += [(prevCoords[0], prevCoords[1]+dist)]
...
for i in range(len(line1coords)-1):
    ...
    line2steps = 0
    for i in range(len(line2coords)-1):
        l2p1 = line2coords[i]
        l2p2 = line2coords[i+1]
        
        if vert and l2p1[0]-l2p2[0] == 0:
            # both vertical
            if l1p1[0] == l2p1[0]:
                # same x
                l1max = max(l1p1[1],l1p2[1])
                l1min = min(l1p1[1],l1p2[1])
                l2max = max(l2p1[1],l2p2[1])
                l2min = min(l2p1[1],l2p2[1])
                
                if l1max > l2min and l1min < l2max:
                    # they overlap
                    overlapMax = l1max if (l1max < l2max) else l2max
                    overlapMin = l1max if (l1min > l2min) else l2min
                    
                    x = l1p1[0]
                    for y in range(overlapMin, overlapMax + 1):
                        steps = line1steps + line2steps + abs(y-l1p1[1]) + abs(y-l2p1[1])
                        if (steps < bestHit or bestHit == -1) and steps > 0:
                            bestHit = steps
                        
        elif (not vert) and l2p1[1]-l2p2[1] == 0:
            # both horizontal
            if l1p1[1] == l2p1[1]:
                # same y
                l1max = max(l1p1[0],l1p2[0])
                l1min = min(l1p1[0],l1p2[0])
                l2max = max(l2p1[0],l2p2[0])
                l2min = min(l2p1[0],l2p2[0])
                ...
                if l1max > l2min and l1min < l2max:
                    ...
                    y = l1p1[1]
                    for x in range(overlapMin, overlapMax + 1):
                        steps = line1steps + line2steps + abs(x-l1p1[0]) + abs(x-l2p1[0])
                        if (steps < bestHit or bestHit == -1) and steps > 0:
                            bestHit = steps
            ...
            if vert:
                x = l1p1[0]
                if x >= min(l2p1[0],l2p2[0]) and x <= max(l2p1[0],l2p2[0]):
                    y = l2p1[1]
                    if y >= min(l1p1[1],l1p2[1]) and y <= max(l1p1[1],l1p2[1]):
                        steps = line1steps + line2steps + abs(y-l1p1[1]) + abs(x-l2p1[0])
                        if (steps < bestHit or bestHit == -1) and steps > 0:
                            bestHit = steps
            else:
                y = l1p1[1]
                if y >= min(l2p1[1],l2p2[1]) and y <= max(l2p1[1],l2p2[1]):
                    x = l2p1[0]
                    if x >= min(l1p1[0],l1p2[0]) and x <= max(l1p1[0],l1p2[0]):
                        steps = line1steps + line2steps + abs(x-l1p1[0]) + abs(y-l2p1[1])
                        if (steps < bestHit or bestHit == -1) and steps > 0:
                            bestHit = steps
                            
        line2steps += abs(l2p2[1]+l2p2[0]-l2p1[1]-l2p1[0])
        if bestHit != -1 and line2steps > bestHit:
            break
    line1steps += abs(l1p2[1]+l1p2[0]-l1p1[1]-l1p1[0])
    if bestHit != -1 and line1steps > bestHit:
        break
